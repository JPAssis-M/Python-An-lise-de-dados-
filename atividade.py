...
def filmes_por_diretor():
    ...
    if "Direcao" not in df.columns:
        return render_template_string("<h2>Coluna 'Direcao' não encontrada nos dados.</h2>")

    # Agrupar por Diretor e contar os filmes
    base = (
        df[df["Direcao"].notna()]
        .groupby("Direcao", as_index=False)
        .agg(QtdeFilmes=("Titulo", "count"))
        .sort_values(by="QtdeFilmes", ascending=False)
    )

    if base.empty:
        return render_template_string("<h2>Sem dados de diretores para exibir.</h2>")

    # Filtrar para os diretores com mais filmes (top 20, por exemplo)
    base_top = base.head(20)
    ...
    fig = px.bar(
        ...
        x="Direcao",
        y="QtdeFilmes",
        title="Quantidade de Filmes por Diretor (Top 20)",
        labels={"Direcao": "Diretor(a)", "QtdeFilmes": "Quantidade de Filmes"},
        text_auto=True
    )
    ...
    html = """
        <h1>Quantidade de Filmes por Diretor</h1>
        <p>Exibindo os 20 diretores com mais filmes.</p>
        <div>{{grafico|safe}}</div>
        <br>
...
def filmes_por_diretor():
    ...
    g: ( df['Direcao'].fillna("").astype(str).str.strip().replace({"N/A":""}))
    g = g[g.ne("")]
    g = g.values_counts().reset_index()
